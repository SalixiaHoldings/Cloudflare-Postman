...
import { assertNoBodySentinel } from './request-body.mjs';
...
export function assertBodyEquivalence(v2, native, label) {
  assertNoBodySentinel(v2, label);
  assertNoBodySentinel(native, label);
  if (!v2) { assert.equal(native, undefined, `Unexpected v3 body: ${label}`); return; }
  const type = v2.mode === 'raw' ? v2.options?.raw?.language ?? 'text' : v2.mode;
  assert.equal(native?.type, type, `Body mode drift: ${label}`);
  if (v2.mode === 'raw') assert.equal(native.content, v2.raw, `Raw body drift: ${label}`);
  else if (v2.mode === 'file') assert.deepEqual(native.content, v2.file ?? {}, `File body drift: ${label}`);
  else if (['formdata', 'urlencoded'].includes(v2.mode)) {
    const rows = Array.isArray(native.content) ? native.content :
      Object.entries(native.content ?? {}).map(([key, value]) => ({ key, value }));
    // Official migration omits empty descriptions/false disabled flags and
    // makes an unselected multipart file's empty src explicit. Compare their
    // wire-equivalent defaults without changing generated bytes.
    const normalized = entries => entries.map(row => ({ ...row, disabled: row.disabled ?? false,
      description: row.description ?? '', ...(row.type === 'file' ? { src: row.src ?? '' } : {}) }));
    assert.deepEqual(normalized(rows), normalized(v2[v2.mode]), `Form body drift: ${label}`);
  } else throw new Error(`Unreviewed body mode: ${v2.mode}`);
}
...
export async function assertSemanticEquivalence(collection, root) {
  ...
  for (const file of await filesUnder(root)) {
    ...
    assertBodyEquivalence(v2.body, request.body, file);
