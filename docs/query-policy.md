...
The current distribution has **139 required and 5,559 optional query contracts**, producing **147 enabled and 8,143 disabled query rows** across **3,594 operations**. Row counts differ from contracts because of repeated array rows and object expansion. The five previously corrupted IAM/Spectrum parameters retain their proper identities.

### One known revision-bound query omission

At revision `7287cb19ea4c5feb93f8dd0a4d8d12872692a802`, the official secondary conversion omits only optional `search` on `GET /accounts/{account_id}/cloudforce-one/events`. This array has `default: []`. Its current parameter SHA-256 is `97be6fa16b6124b7a7217c1fead4c42aaa9e0657d6337cd3ccd7a152d6b73ffb`, freshly measured from the pinned schema. The [superseding PR decision](https://github.com/SalixiaHoldings/Cloudflare-Postman/pull/7#issuecomment-5731810896) approves this exact limitation.
...
The [7287cb19 revision review](schema-revision-7287cb19.md) re-evaluates the omission and every secondary warning against the new source. Independent full secondary conversions and operation-attributed warning capture preserve all ten partition fingerprints and all 452 diagnostics exactly. No omission contract, warning fingerprint, or query serializer changes.
