...
The converter is a build-only dependency and processes the exact pinned Cloudflare schema. Patch-compatible overrides keep its `js-yaml`, `yaml`, and `uuid` dependencies on fixed releases. On September 15, 2026, `npm audit --json` reports four high-severity affected packages, representing two distinct advisories (not four independent advisories):

- [Faker `helpers.fake` code execution, GHSA-qxc2-j82w-r537](https://github.com/advisories/GHSA-qxc2-j82w-r537): `openapi-to-postmanv2 → postman-collection → @faker-js/faker@5.5.3`. This accounts for Faker and two dependent-package findings. This pipeline does not call `helpers.fake`; forcing a new major Faker API into the legacy Postman SDK is not an accepted fix.
- [js-yaml merge-source CPU exhaustion, GHSA-2883-xcg3-v3hh / CVE-2026-84375](https://github.com/advisories/GHSA-2883-xcg3-v3hh): `openapi-to-postmanv2 → js-yaml@4.3.1`. This adds the fourth affected package; the converter finding now also includes this dependency. The advisory was indexed/reviewed by GitHub on September 8 and affects versions before 4.3.2. The dependency lock and resolution are unchanged from the foundation release. Generation verifies the official JSON bytes by SHA-256, parses them with `JSON.parse`, and passes a JSON object to the converter; it does not accept attacker-supplied YAML merge sources. The dependencies are not shipped inside the generated Postman artifacts.

These remain accepted limitations for this pinned-input build, not evidence that either dependency is generally safe. A compatible js-yaml patch exists and can be evaluated in a separate toolchain update; this schema synchronization keeps the established toolchain fixed. Do not run forced audit fixes or relax the input boundary. Reassess before adding arbitrary YAML input or exposing conversion as a service.
