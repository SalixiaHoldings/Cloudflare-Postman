...
PR #4's subsequent live-query acceptance work is paused on a [reproduced converter query-identity defect](query-policy-blocker.md). The requested global optional-query defaults and tenant/organization template additions remain pending that review; Desktop acceptance is not the only remaining gate.
