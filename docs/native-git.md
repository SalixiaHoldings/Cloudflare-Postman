...
The [request-body contract](request-body-generation-audit.md) recursively checks writable request schemas and rejects converter sentinels in live bodies. The manifest separately reports proven `ambiguous-oneOf` and `source-incomplete` source conditions; incomplete templates carry a visible request-description warning and are not reported as schema-valid templates.
