...
test('pinned distribution preserves query contracts, exact omissions, Organizations and five repaired identities', async () => {
  ...
  assertOmissions(omissions,policy.omissions);assert.equal(items.size,3594);assert.equal(enabled,147);assert.equal(disabled,8143);
  ...
  assert.equal(manifest.partitions.reduce((n,p)=>n+p.queryProjection.secondaryWarnings.count,0),452);
});
